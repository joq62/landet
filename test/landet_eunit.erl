...
%    ok=brytar_test("brytare_2"),
  ...
  %  timer:sleep(2000),
    ...
    ok=test_switch("brytare_1",wait_on),
...
test_switch("brytare_1",wait_off)->
    io:format("test_switch(brytare_1,wait_off)  ~p~n",[?LINE]),
    true=pushed_off("brytare_1",true), 
    io:format("true=pushed_off(brytare_1,true), ~p~n",[?LINE]),
    spawn(fun()->tradfri_bulb_E14_ws_candleopal_470lm:turn_off("hallen_byran_lampa") end),	    
    spawn(fun()->tradfri_bulb_e27_ww_806lm:turn_off("vrum_bordslampa") end),
    spawn(fun()->tradfri_bulb_e27_ww_806lm:turn_off("vrum_dim_golv_soffa") end),
    test_switch("brytare_1",wait_on);

test_switch("brytare_1",wait_on)->
    io:format("test_switch(brytare_1,wait_on)  ~p~n",[?LINE]),
    true=pushed_on("brytare_1",false),
    io:format("true=pushed_on(brytare_1,false), ~p~n",[?LINE]),
    spawn(fun()->tradfri_bulb_E14_ws_candleopal_470lm:turn_on("hallen_byran_lampa",78,443) end),	    
    spawn(fun()->tradfri_bulb_e27_ww_806lm:turn_on("vrum_bordslampa",78) end),
    spawn(fun()->tradfri_bulb_e27_ww_806lm:turn_on("vrum_dim_golv_soffa",78) end),
    test_switch("brytare_1",wait_off);
